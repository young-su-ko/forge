...
class FlowSimulator:
    ...
    def sample(
        ...
        lengths = torch.full((n_samples,), fill_value=length, device=self.device)
...
class ValFlowSimulator:
    def __init__(self, velocity_model, guidance_scale=0.0, t_steps=100):
        ...
        self.length_predictor.load_state_dict(
            torch.load(
                "/new-stg/home/young/raygun-length/length_predictor_weights_8_8.pt",
                map_location="cpu",
            )
        )
